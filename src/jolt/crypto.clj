...
(defn- ->ba [x]
  (cond
    (and (table? x) (#{:jolt.crypto/key :jolt.crypto/iv
                       :jolt.crypto/ec-public :jolt.crypto/ec-private :jolt.crypto/public-key
...
(defn install! []
    ...
    {"getType" (fn [self] "X.509")
     ...
     "getPublicKey" (fn [self]
                      (let [{:keys [algorithm der]} (tget self :public-key)]
                        (if (= "EC" algorithm)
                          (doto (tt :jolt.crypto/ec-public) (tput! :bytes der))
                          (doto (tt :jolt.crypto/public-key) (tput! :bytes der) (tput! :algorithm algorithm)))))
  ...
  (__register-class-methods! :jolt.crypto/public-key
    {"getEncoded" (fn [self] (tget self :bytes))
     "getAlgorithm" (fn [self] (tget self :algorithm))
     "getFormat" (fn [self] "X.509")})
